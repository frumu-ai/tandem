fn incident_monitor_triage_manual_schedule() -> crate::AutomationV2Schedule {
...
fn incident_monitor_triage_output_contract(
    _artifact_type: &str,
...
fn incident_monitor_triage_node_artifact_type(node: &crate::AutomationFlowNode) -> Option<&str> {
    node.metadata
        .as_ref()?
...
fn incident_monitor_triage_expected_contract(
    artifact_type: &str,
    _objective: &str,
) -> crate::AutomationFlowOutputContract {
    incident_monitor_triage_output_contract(
        artifact_type,
        &incident_monitor_triage_repo_evidence_guidance(artifact_type),
        true,
    )
}

fn incident_monitor_triage_node_contract_is_stale(node: &crate::AutomationFlowNode) -> bool {
    let Some(artifact_type) = incident_monitor_triage_node_artifact_type(node) else {
        return false;
    };
    let expected = incident_monitor_triage_expected_contract(artifact_type, &node.objective);
...
pub(crate) fn incident_monitor_triage_flow_has_stale_output_contracts(
    flow: &crate::AutomationFlowSpec,
) -> bool {
    flow.nodes
        .iter()
        .any(incident_monitor_triage_node_contract_is_stale)
}

pub(crate) fn normalize_incident_monitor_triage_output_contracts(spec: &mut crate::AutomationV2Spec) {
...
fn incident_monitor_triage_repo_evidence_guidance(artifact_type: &str) -> String {
    format!(
        "Required output: valid completed JSON for `{artifact_type}` in the final response. Do not write Incident Monitor artifacts into the workspace; Tandem will store the accepted JSON in the global context-run artifact store. Before responding, perform the best available local repo evidence pass inside the configured `workspace_root` from Node Inputs: call `codesearch`, `grep`, or `glob` for focused symbols, node IDs, error strings, event names, artifact paths, and workflow IDs from the payload, then prefer calling `read` on at least one concrete workspace source file discovered by that search. If no relevant code is found, or if `read` cannot be completed, still return a completed JSON artifact that preserves the search evidence, tool limitations, uncertainty, and the original workflow failure. Do not block Incident Monitor reporting solely because source reads were inconclusive. Include `workspace_root`, `search_queries_used`, `files_examined`, `file_references` with path and line/snippet when available, `likely_files_to_edit`, `affected_components`, `tool_evidence`, `uncertainty`, and `bounded_next_steps`. Do not finish with only generic diagnosis."
    )
}

fn incident_monitor_triage_node(
    node_id: &str,
    agent_id: &str,
    objective: &str,
    ...
    crate::AutomationFlowNode {
        ...
        depends_on,
        input_refs: Vec::new(),
        output_contract: Some(incident_monitor_triage_output_contract(
            artifact_type,
            &incident_monitor_triage_repo_evidence_guidance(artifact_type),
            true,
        )),
...
pub(crate) fn incident_monitor_triage_spec(
    ...
    let automation_id = format!("automation-v2-incident-monitor-triage-{}", draft.draft_id);
    ...
    crate::AutomationV2Spec {
        automation_id,
        name: format!(
            "Incident Monitor triage: {}",
            draft.title.as_deref().unwrap_or("runtime failure").trim()
        ),
        description: Some(format!(
            "Inspect, research, validate, and propose a fix for Incident Monitor draft {}.",
            draft.draft_id
        )),
        status: crate::AutomationV2Status::Active,
        schedule: incident_monitor_triage_manual_schedule(),
        knowledge: tandem_orchestrator::KnowledgeBinding::default(),
        agents: vec![crate::AutomationAgentProfile {
            agent_id: "incident_monitor_triage_agent".to_string(),
            template_id: None,
            display_name: "Incident Monitor Triage".to_string(),
            avatar_url: None,
            model_policy,
            skills: Vec::new(),
        ...
        flow: crate::AutomationFlowSpec {
            nodes: vec![
                incident_monitor_triage_node(
                    "inspect_failure_report",
                    "incident_monitor_triage_agent",
        ...
        created_at_ms: now,
        updated_at_ms: now,
        creator_id: "incident_monitor".to_string(),
        workspace_root,
        metadata: Some(json!({
...
pub(crate) fn incident_monitor_automation_run_id_from_triage_run_id(
    triage_run_id: &str,
) -> Option<String> {
    triage_run_id
...
pub(crate) fn incident_monitor_triage_context_run_id(run_id: &str) -> String {
    super::context_runs::automation_v2_context_run_id(run_id)
}

fn incident_monitor_automation_run_is_terminal_for_triage(status: &crate::AutomationRunStatus) -> bool {
    matches!(
        status,
...
pub(crate) async fn incident_monitor_triage_effective_started_at_ms(
    state: &AppState,
    triage_run_id: &str,
) -> Option<u64> {
    if let Some(run_id) = incident_monitor_automation_run_id_from_triage_run_id(triage_run_id) {
        return state
...
pub(crate) async fn incident_monitor_triage_run_is_terminal(state: &AppState, run_id: &str) -> bool {
    if let Some(automation_run_id) = incident_monitor_automation_run_id_from_triage_run_id(run_id) {
        return state
            .get_automation_v2_run(&automation_run_id)
            .await
            .map(|run| incident_monitor_automation_run_is_terminal_for_triage(&run.status))
            .unwrap_or(false);
    }
...
pub(crate) async fn incident_monitor_triage_run_is_reusable(state: &AppState, run_id: &str) -> bool {
    if let Some(automation_run_id) = incident_monitor_automation_run_id_from_triage_run_id(run_id) {
        return state
            .get_automation_v2_run(&automation_run_id)
            .await
...
pub(crate) async fn finalize_completed_incident_monitor_triage(
    state: &AppState,
    draft_id: &str,
) -> anyhow::Result<bool> {
    let Some(draft) = state.get_incident_monitor_draft(draft_id).await else {
        return Ok(false);
    };
    ...
    let Some(automation_run_id) = incident_monitor_automation_run_id_from_triage_run_id(&triage_run_id)
    else {
        return Ok(false);
    };
    ...
    if !incident_monitor_automation_run_is_terminal_for_triage(&run.status) {
        return Ok(false);
    }

    let incident_id = incident_monitor_incident_id_for_draft(state, draft_id, &triage_run_id).await;
    if load_incident_monitor_triage_summary_artifact(state, &triage_run_id)
        .await
        .is_none()
    {
        let response = create_incident_monitor_triage_summary(
            State(state.clone()),
            Path(draft_id.to_string()),
            Json(IncidentMonitorTriageSummaryInput::default()),
        )
        .await;
        if !response.status().is_success() {
            anyhow::bail!(
                "Incident Monitor triage summary finalization failed with HTTP {}",
                response.status()
            );
        }
    ...
    .await
    {
        Ok(outcome) => {
            if let Some(incident_id) = incident_id {
                if let Some(mut incident) = state.get_incident_monitor_incident(&incident_id).await {
                    incident.status = outcome.action.clone();
                    incident.last_error = None;
                    incident.updated_at_ms = crate::now_ms();
                    let _ = state.put_incident_monitor_incident(incident).await;
                }
            }
            Ok(true)
        }
        Err(error) => {
            ...
            if let Some(mut draft) = state.get_incident_monitor_draft(draft_id).await {
                ...
                let _ = state.put_incident_monitor_draft(draft.clone()).await;
                if let Err(record_err) = crate::incident_monitor::router::record_publish_failure(
                    state,
                    &draft,
                ...
                .await
                {
                    tracing::warn!(
                        draft_id = %draft_id,
                        error = %record_err,
                        "failed to record Incident Monitor triage finalization post failure",
                    );
                }
            }
            if let Some(incident_id) = incident_id {
...
async fn incident_monitor_incident_id_for_draft(
    state: &AppState,
    draft_id: &str,
    triage_run_id: &str,
) -> Option<String> {
    let incidents = state.incident_monitor_incidents.read().await;
    incidents
        .values()
        .find(|incident| {
...
pub(crate) async fn incident_monitor_triage_timeout_diagnostics(
    state: &AppState,
    run_id: &str,
    timeout_ms: u64,
) -> Option<serde_json::Value> {
    if let Some(automation_run_id) = incident_monitor_automation_run_id_from_triage_run_id(run_id) {
        return incident_monitor_automation_triage_timeout_diagnostics(
            state,
            &automation_run_id,
            timeout_ms,
        )
    ...
    super::context_runs::incident_monitor_triage_timeout_diagnostics(state, run_id, timeout_ms).await
}

async fn incident_monitor_automation_triage_timeout_diagnostics(
    state: &AppState,
    run_id: &str,
    timeout_ms: u64,
) -> Option<serde_json::Value> {
    ...
    Some(json!({
        "run_id": run.run_id,
        "context_run_id": incident_monitor_triage_context_run_id(&run.run_id),
        "run_status": status,
...
#[cfg(test)]
mod incident_monitor_triage_spec_tests {
    use super::*;
    ...
    fn triage_spec_has_four_nodes_with_correct_dependencies() {
        ...
        let spec = incident_monitor_triage_spec(
            &draft,
    ...
    fn triage_flow_detects_and_normalizes_stale_inspection_contract() {
        ...
        let mut spec = incident_monitor_triage_spec(
            &draft,
            Some("/tmp/workspace".to_string()),
            None,
            Vec::new(),
        ...
        inspect_node.output_contract = Some(incident_monitor_triage_output_contract(
            "incident_monitor_inspection",
            "legacy guidance",
            true,
        ));

        assert!(incident_monitor_triage_flow_has_stale_output_contracts(
            &spec.flow
        ));

        normalize_incident_monitor_triage_output_contracts(&mut spec);
    ...
    fn triage_terminal_status_treats_blocked_as_fallback_publishable() {
        ...
        assert!(incident_monitor_automation_run_is_terminal_for_triage(
            &Completed
        ));
        assert!(incident_monitor_automation_run_is_terminal_for_triage(&Failed));
        assert!(incident_monitor_automation_run_is_terminal_for_triage(
            &Cancelled
        ));
        assert!(incident_monitor_automation_run_is_terminal_for_triage(&Blocked));
        assert!(!incident_monitor_automation_run_is_terminal_for_triage(&Queued));
        assert!(!incident_monitor_automation_run_is_terminal_for_triage(&Running));
        assert!(!incident_monitor_automation_run_is_terminal_for_triage(&Paused));
    }
}
