const BUG_MONITOR_DEPLOYMENT_CARDS_SCHEMA_VERSION: u64 = 1;

const BUG_MONITOR_DEPLOYMENT_CARD_REQUIRED_FIELDS: [&str; 8] = [
    "intended_purpose",
    "business_owner",
    "accountable_team",
    "autonomy_level",
...
pub(super) struct BugMonitorDeploymentCardMetadata {
...
pub(super) struct BugMonitorDeploymentCardsInput {
    #[serde(default)]
    pub defaults: BugMonitorDeploymentCardMetadata,
    #[serde(default)]
    pub metadata: std::collections::BTreeMap<String, BugMonitorDeploymentCardMetadata>,
    #[serde(default)]
...
struct BugMonitorDeploymentCardSeed {
    card_id: String,
    card_kind: String,
    target_kind: String,
    target_id: String,
...
pub(super) async fn generate_bug_monitor_deployment_cards(
    State(state): State<AppState>,
    Extension(tenant_context): Extension<TenantContext>,
    verified_tenant_context: Option<Extension<VerifiedTenantContext>>,
    headers: HeaderMap,
    Json(input): Json<BugMonitorDeploymentCardsInput>,
) -> Response {
    ...
    let payload = bug_monitor_deployment_cards_payload(
        &state,
    ...
    bug_monitor_assessment_report_emit_audit_event(
        &state,
        &tenant_context,
        "bug_monitor.deployment_cards.generated",
        json!({
...
async fn bug_monitor_deployment_cards_payload(
    state: &AppState,
    ...
    let policy = BugMonitorPostureRulePolicy {
        ...
        min_severity_rank: bug_monitor_posture_severity_rank("low"),
    };
    let posture_findings = bug_monitor_security_posture_findings(&authority_inventory, &policy);
    ...
    cards.push(bug_monitor_deployment_card_from_seed(
        bug_monitor_deployment_card_seed_for_monitor(&inventory, &tenant_context),
        input,
        &posture_findings,
        &mut findings,
    ));
    ...
    for automation in bug_monitor_posture_array(&inventory, &["automation_specs"]) {
        cards.push(bug_monitor_deployment_card_from_seed(
            bug_monitor_deployment_card_seed_for_automation(automation),
            input,
            &posture_findings,
            &mut findings,
        ));
    }
    for workflow in bug_monitor_posture_array(&inventory, &["workflows"]) {
        cards.push(bug_monitor_deployment_card_from_seed(
            bug_monitor_deployment_card_seed_for_workflow(workflow),
            input,
            &posture_findings,
            &mut findings,
        ));
    ...
    for source in bug_monitor_posture_array(&inventory, &["monitored_sources"]) {
        cards.push(bug_monitor_deployment_card_from_seed(
            bug_monitor_deployment_card_seed_for_source(source),
            input,
            &posture_findings,
            &mut findings,
        ));
    ...
    json!({
        ...
        "card_policy": {
            "required_fields": BUG_MONITOR_DEPLOYMENT_CARD_REQUIRED_FIELDS,
        ...
        "counts": {
            "cards": cards.len(),
            "by_card_kind": bug_monitor_assessment_report_count_values(&cards, "card_kind"),
        ...
        "sensitive_values": {
            ...
            "omitted_fields": [
                "bug_monitor_intake_key.key_hash",
                "bug_monitor_intake_key.raw_key",
...
fn bug_monitor_deployment_card_seed_for_monitor(
    inventory: &Value,
    tenant_context: &TenantContext,
) -> BugMonitorDeploymentCardSeed {
    let bug_monitor = inventory.get("bug_monitor").cloned().unwrap_or(Value::Null);
    let destinations = bug_monitor_posture_array(inventory, &["destinations"])
        .iter()
        .filter_map(|row| bug_monitor_deployment_card_value_text(row, "destination_id"))
        .collect::<Vec<_>>();
    let routes = bug_monitor_posture_array(inventory, &["routes"])
        .iter()
        .filter_map(|row| bug_monitor_deployment_card_value_text(row, "route_id"))
        .collect::<Vec<_>>();
    BugMonitorDeploymentCardSeed {
        card_id: "tandem:self_monitoring:bug_monitor".to_string(),
        card_kind: "tandem_self_monitoring".to_string(),
        target_kind: "bug_monitor".to_string(),
        target_id: "bug_monitor".to_string(),
        ...
        destinations,
        routes,
        data_sources: bug_monitor_posture_array(inventory, &["monitored_sources"])
            .iter()
            .filter_map(bug_monitor_deployment_card_source_ref)
            .collect::<Vec<_>>(),
        tenant_scope: json!({
        ...
        evidence_refs: vec!["authority_inventory:bug_monitor".to_string()],
        authority: json!({
            "bug_monitor": bug_monitor,
            "configured_destinations": bug_monitor_posture_array(inventory, &["destinations"]).len(),
            "routes": bug_monitor_posture_array(inventory, &["routes"]).len(),
            "monitored_sources": bug_monitor_posture_array(inventory, &["monitored_sources"]).len(),
        }),
    }
}

fn bug_monitor_deployment_card_seed_for_automation(
    automation: &Value,
) -> BugMonitorDeploymentCardSeed {
    let automation_id = bug_monitor_deployment_card_value_text(automation, "automation_id")
        .unwrap_or_else(|| "unknown_automation".to_string());
    let target_name = bug_monitor_deployment_card_value_text(automation, "name")
    ...
    let output_targets =
        bug_monitor_deployment_card_value_string_array(automation, "output_targets");
    BugMonitorDeploymentCardSeed {
        ...
        fallback_purpose: bug_monitor_deployment_card_value_text(automation, "description"),
...
fn bug_monitor_deployment_card_seed_for_workflow(workflow: &Value) -> BugMonitorDeploymentCardSeed {
    ...
    for hook in bug_monitor_posture_array(workflow, &["hooks"]) {
        for action in bug_monitor_posture_array(hook, &["actions"]) {
            if let Some(action_name) = bug_monitor_deployment_card_value_text(action, "action") {
    ...
    BugMonitorDeploymentCardSeed {
        ...
        fallback_purpose: bug_monitor_deployment_card_value_text(workflow, "description"),
...
fn bug_monitor_deployment_card_seed_for_source(source: &Value) -> BugMonitorDeploymentCardSeed {
    ...
    BugMonitorDeploymentCardSeed {
        ...
        system_boundary: bug_monitor_deployment_card_boundary(source_kind.as_deref()).to_string(),
        ...
        mcp_servers: bug_monitor_deployment_card_value_text(source, "mcp_server")
            ...
            .collect(),
        destinations,
        routes: bug_monitor_deployment_card_value_string_array(source, "default_route_tags"),
        ...
        authority: json!({
            ...
            "redaction_profile_present": bug_monitor_deployment_card_value_has_text(source, "redaction_profile"),
            "retention_profile_present": bug_monitor_deployment_card_value_has_text(source, "retention_profile"),
            "event_schema_version": source.get("event_schema_version").cloned().unwrap_or(Value::Null),
        }),
    }
}

fn bug_monitor_deployment_card_from_seed(
    ...
    let purpose = bug_monitor_deployment_card_metadata_text(
        metadata,
        &input.defaults,
        |metadata| &metadata.intended_purpose,
        seed.fallback_purpose.clone(),
    );
    let business_owner = bug_monitor_deployment_card_metadata_text(
        metadata,
        &input.defaults,
        |metadata| &metadata.business_owner,
        None,
    );
    let accountable_team = bug_monitor_deployment_card_metadata_text(
        metadata,
        &input.defaults,
        |metadata| &metadata.accountable_team,
        None,
    );
    let autonomy_level = bug_monitor_deployment_card_metadata_text(
        metadata,
        &input.defaults,
        |metadata| &metadata.autonomy_level,
        None,
    );
    let data_classification = bug_monitor_deployment_card_metadata_text(
        metadata,
        &input.defaults,
        |metadata| &metadata.data_classification,
        None,
    );
    let approval_protocol = bug_monitor_deployment_card_metadata_text(
        metadata,
        &input.defaults,
        |metadata| &metadata.approval_protocol,
        None,
    );
    let escalation_protocol = bug_monitor_deployment_card_metadata_text(
        metadata,
        &input.defaults,
    ...
    let allowed_actions = bug_monitor_deployment_card_metadata_list(
        metadata,
        &input.defaults,
        |metadata| &metadata.allowed_actions,
        seed.allowed_actions,
    );
    let prohibited_actions = bug_monitor_deployment_card_metadata_list(
        metadata,
        &input.defaults,
        |metadata| &metadata.prohibited_actions,
        seed.prohibited_actions,
    );
    let known_limitations = bug_monitor_deployment_card_metadata_list(
        metadata,
        &input.defaults,
    ...
    let residual_risks = bug_monitor_deployment_card_metadata_list(
        metadata,
        &input.defaults,
        |metadata| &metadata.residual_risks,
        vec!["Incomplete operator metadata reduces production governance confidence.".to_string()],
    );
    let operator_evidence_refs = bug_monitor_deployment_card_metadata_list(
        metadata,
        &input.defaults,
...
fn bug_monitor_deployment_card_missing_findings(
    seed: &BugMonitorDeploymentCardSeed,
...
fn bug_monitor_deployment_card_metadata_text<F>(
    metadata: Option<&BugMonitorDeploymentCardMetadata>,
    defaults: &BugMonitorDeploymentCardMetadata,
    pick: F,
    fallback: Option<String>,
) -> Option<String>
where
    F: Fn(&BugMonitorDeploymentCardMetadata) -> &Option<String>,
{
    metadata
        .and_then(|metadata| bug_monitor_deployment_card_clean_text(pick(metadata)))
        .or_else(|| bug_monitor_deployment_card_clean_text(pick(defaults)))
        .or(fallback)
}

fn bug_monitor_deployment_card_metadata_list<F>(
    metadata: Option<&BugMonitorDeploymentCardMetadata>,
    defaults: &BugMonitorDeploymentCardMetadata,
    pick: F,
    fallback: Vec<String>,
...
where
    F: Fn(&BugMonitorDeploymentCardMetadata) -> &Vec<String>,
{
    if let Some(metadata) = metadata {
        let values = bug_monitor_deployment_card_clean_list(pick(metadata));
        if !values.is_empty() {
            return values;
        }
    }
    let values = bug_monitor_deployment_card_clean_list(pick(defaults));
    if values.is_empty() {
        fallback
    } else {
        values
...
fn bug_monitor_deployment_card_clean_text(value: &Option<String>) -> Option<String> {
    value
        .as_deref()
        .map(str::trim)
...
fn bug_monitor_deployment_card_clean_list(values: &[String]) -> Vec<String> {
    values
        .iter()
...
fn bug_monitor_deployment_card_value_text(value: &Value, key: &str) -> Option<String> {
    value
        .get(key)
...
fn bug_monitor_deployment_card_value_has_text(value: &Value, key: &str) -> bool {
    bug_monitor_deployment_card_value_text(value, key).is_some()
}

fn bug_monitor_deployment_card_value_string_array(value: &Value, key: &str) -> Vec<String> {
    value
        .get(key)
        .and_then(Value::as_array)
        .map(|rows| {
...
fn bug_monitor_deployment_card_source_ref(source: &Value) -> Option<String> {
    let project_id = bug_monitor_deployment_card_value_text(source, "project_id")?;
    let source_id = bug_monitor_deployment_card_value_text(source, "source_id")
...
fn bug_monitor_deployment_card_extend_text_set(
...
fn bug_monitor_deployment_card_boundary(source_kind: Option<&str>) -> &'static str {
    match source_kind {
...
fn bug_monitor_deployment_card_value_contains_text(value: &Value, needle: &str) -> bool {
    ...
    match value {
        ...
        Value::Array(rows) => rows
            .iter()
            .any(|row| bug_monitor_deployment_card_value_contains_text(row, needle)),
...
fn bug_monitor_deployment_cards_markdown(
    generated_at_ms: u64,
    tenant_context: &TenantContext,
    cards: &[Value],
    findings: &[Value],
