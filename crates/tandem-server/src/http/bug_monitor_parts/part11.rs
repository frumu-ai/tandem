const BUG_MONITOR_ASSESSMENT_PROBES_SCHEMA_VERSION: u64 = 1;

const BUG_MONITOR_ASSESSMENT_PROBES: [&str; 6] = [
...
pub(super) struct BugMonitorAssessmentProbeRunInput {
    #[serde(default)]
...
pub(super) async fn run_bug_monitor_security_assessment_probes(
    ...
    let mode = bug_monitor_assessment_mode(Some(requested_mode));
    ...
    let probe_policy = json!({
        ...
        "supported_probe_ids": BUG_MONITOR_ASSESSMENT_PROBES,
    ...
    let mut persisted_payload = json!({
        "schema_version": BUG_MONITOR_ASSESSMENT_PROBES_SCHEMA_VERSION,
    ...
    let evidence_pack = if dry_run {
        match bug_monitor_assessment_persist_evidence_pack(
            &state,
    ...
    emit_bug_monitor_admin_audit_event(
        &state,
        "bug_monitor.assessment_probes.completed",
        json!({
...
fn bug_monitor_assessment_probe_posture_rule(
    ...
    let findings = bug_monitor_assessment_posture_findings(authority_inventory, posture_rule_id);
...
fn bug_monitor_assessment_probe_route_preview_high_risk(
    ...
    if preview.effective_destination_ids.is_empty() {
        results.push(bug_monitor_assessment_result(
            "route_preview_high_risk_approval",
            json!({
                "kind": "bug_monitor_route_preview",
            ...
            vec![bug_monitor_posture_evidence_ref(
                "bug_monitor.route_preview[security.assessment.probe]",
            )],
            None,
            include_draft_suggestions,
        ));
    ...
    results.push(bug_monitor_assessment_result(
        ...
        vec![
            bug_monitor_posture_evidence_ref(
                "bug_monitor.route_preview[security.assessment.probe].approval_required",
            ),
            bug_monitor_posture_evidence_ref(
...
fn bug_monitor_assessment_probe_destination_readiness(
    status: &crate::BugMonitorStatus,
    include_draft_suggestions: bool,
    results: &mut Vec<Value>,
) {
    ...
    for readiness in &status.destination_readiness {
        ...
        if readiness.publish_ready {
            results.push(bug_monitor_assessment_result(
                ...
                "pass",
                vec![bug_monitor_posture_evidence_ref(format!(
                    "bug_monitor.status.destination_readiness[{}]",
...
fn bug_monitor_assessment_probe_scoped_intake(
    authority_inventory: &Value,
    include_draft_suggestions: bool,
    results: &mut Vec<Value>,
) {
    let keys = bug_monitor_posture_array(
        authority_inventory
            .get("inventory")
            .unwrap_or(&Value::Null),
        &["scoped_intake_keys"],
    ...
    for key in keys {
        ...
        evaluated += 1;
        let key_id = bug_monitor_posture_str(key, "key_id").unwrap_or("unknown_intake_key");
        ...
        results.push(bug_monitor_assessment_result(
            ...
            vec![
                ...
                bug_monitor_posture_evidence_ref(
                    "bug_monitor.security.assessment_probes.intake_key_rejection",
                ),
            ],
            None,
            include_draft_suggestions,
...
fn bug_monitor_assessment_probe_mcp_allowlist(
    authority_inventory: &Value,
    status: &crate::BugMonitorStatus,
    include_draft_suggestions: bool,
    results: &mut Vec<Value>,
) {
    bug_monitor_assessment_probe_posture_rule(
        authority_inventory,
        "mcp_tool_allowlist",
    ...
    for readiness in &status.destination_readiness {
        if !matches!(&readiness.kind, BugMonitorDestinationKind::McpTool) || !readiness.enabled {
            continue;
        }
    ...
    if evaluated_destinations == 0 {
        results.push(bug_monitor_assessment_result(
            ...
            "blocked",
            vec![bug_monitor_posture_evidence_ref(
                "bug_monitor.status.destination_readiness[kind=mcp_tool]",
            )],
            None,
            include_draft_suggestions,
        ));
...
fn bug_monitor_assessment_probe_webhook_url_policy(
    status: &crate::BugMonitorStatus,
    include_draft_suggestions: bool,
    results: &mut Vec<Value>,
) {
    let mut evaluated = 0usize;
    for readiness in &status.destination_readiness {
        if !matches!(&readiness.kind, BugMonitorDestinationKind::Webhook) || !readiness.enabled {
            continue;
        }
        evaluated += 1;
        results.push(bug_monitor_assessment_result(
    ...
    if evaluated == 0 {
        results.push(bug_monitor_assessment_result(
            ...
            "blocked",
            vec![bug_monitor_posture_evidence_ref(
                "bug_monitor.status.destination_readiness[kind=webhook]",
            )],
            None,
            include_draft_suggestions,
        ));
...
fn bug_monitor_assessment_posture_findings(
    authority_inventory: &Value,
    posture_rule_id: &str,
) -> Vec<Value> {
    ...
    bug_monitor_security_posture_findings(authority_inventory, &policy)
        .into_iter()
        .filter(|finding| finding.get("rule_id").and_then(Value::as_str) == Some(posture_rule_id))
        .collect()
}

fn bug_monitor_assessment_result(
    probe_id: &str,
    target: Value,
    expected_behavior: &str,
...
fn bug_monitor_assessment_counts(results: &[Value]) -> Value {
...
fn bug_monitor_assessment_selected_probe_ids(requested: &[String]) -> Vec<String> {
    let mut selected = if requested.is_empty() {
        BUG_MONITOR_ASSESSMENT_PROBES
            .iter()
        ...
        requested
            ...
            .filter(|value| BUG_MONITOR_ASSESSMENT_PROBES.iter().any(|probe| probe == value))
            .map(ToString::to_string)
            .collect::<Vec<_>>()
    };
    selected.sort();
...
fn bug_monitor_assessment_mode(mode: Option<&str>) -> String {
    match mode
        .unwrap_or("dry_run")
        .trim()
        .to_ascii_lowercase()
...
async fn bug_monitor_assessment_persist_evidence_pack(
    state: &AppState,
    ...
    let run_id = format!(
        "bug-monitor-assessment-probes-{}-{}",
        generated_at_ms,
        Uuid::new_v4().simple()
    );
    let artifact_id = format!("bug-monitor-assessment-probes-{}", Uuid::new_v4().simple());
    super::context_runs::context_run_create_impl(
        state.clone(),
        tenant_context,
        ContextRunCreateInput {
            run_id: Some(run_id.clone()),
            objective: "Run Bug Monitor security assessment probes and persist evidence.".to_string(),
            run_type: Some("bug_monitor_assessment_probes".to_string()),
    ...
    write_bug_monitor_artifact(
        ...
        &run_id,
        &artifact_id,
        "bug_monitor_assessment_probe_results",
        "artifacts/bug_monitor.assessment_probes.json",
        payload,
    )
    .await?;
    ...
    Ok(json!({
        ...
        "artifact_type": "bug_monitor_assessment_probe_results",
        "path": context_run_dir(state, &run_id)
            .join("artifacts/bug_monitor.assessment_probes.json")
            .to_string_lossy()
            .to_string(),
    }))
}

fn bug_monitor_assessment_request_has_api_token(headers: &HeaderMap, required: &str) -> bool {
    if headers
        .get("x-agent-token")
        .and_then(|value| value.to_str().ok())
        .map(str::trim)
