async fn evaluate_enterprise_authored_tool_policy(
    ...
    use tandem_enterprise_contract::{
        AccessPermission, EnterprisePolicyEffect, EnterprisePolicyInput,
        EnterprisePolicyResolver, EnterprisePolicyRuleState,
    };
    ...
    let snapshot = EnterprisePolicyResolver::new(relevant_rules).resolve(&input, resolved_at_ms);
    ...
    Ok(match snapshot.effect {
        ...
        EnterprisePolicyEffect::ApprovalRequired => Some(ToolPolicyDecision {
            allowed: false,
            reason: Some(format!("{reason}; approval receipt required before execution")),
            policy_decision_id: Some(decision_id),
        }),
    })
}
...
mod enterprise_authored_policy_tests {
    ...
    async fn authored_parameter_policy_matches_preview_and_records_runtime_decisions() {
        ...
        assert!(allowed.is_none(), "matching authored allow continues to lower guards");
        ...
        assert_eq!(recorded.approval_id.as_deref(), Some("finance-large-refund"));
        let preview = state
