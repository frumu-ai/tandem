...
pub use credential_lifecycle::{
    ...
    provider_credential_revision_for_tenant_in_dir, ProviderCredentialKind,
...
fn write_secure_json(path: &PathBuf, value: &Value) -> anyhow::Result<()> {
    ...
    // NamedTempFile creates a private file and persist atomically replaces the
    // destination on supported platforms. Sync before publishing the new inode.
...
fn save_provider_index(ids: &HashSet<String>) -> anyhow::Result<()> {
    credential_lifecycle::save_index(&provider_auth_index_path(), ids)
}

fn save_provider_credentials_index(ids: &HashSet<String>) -> anyhow::Result<()> {
    credential_lifecycle::save_index(&provider_credentials_index_path(), ids)
}
...
fn save_provider_index_to_dir(security_dir: &Path, ids: &HashSet<String>) -> anyhow::Result<()> {
    credential_lifecycle::save_index(&security_dir.join("provider_auth_index.json"), ids)
}
...
fn save_provider_credentials_index_to_dir(
    ...
    credential_lifecycle::save_index(&security_dir.join("provider_credentials_index.json"), ids)
}
...
include!("provider_auth_store/storage_operations.rs");
