"""
tandem_client — Python client for the Tandem autonomous agent engine.

Full coverage of the Tandem HTTP + SSE API.

Async (recommended)::

    from tandem_client import TandemClient

    async with TandemClient(base_url="http://localhost:39731", token="...") as client:
        session_id = await client.sessions.create(title="My agent")
        run = await client.sessions.prompt_async(session_id, "Summarize README.md")
        async for event in client.stream(session_id, run.run_id):
            if event.type == "session.response":
                print(event.properties.get("delta", ""), end="", flush=True)
            if event.type in ("run.complete", "run.completed", "run.failed", "session.run.finished"):
                break

Sync (scripts)::

    from tandem_client import SyncTandemClient

    client = SyncTandemClient(base_url="http://localhost:39731", token="...")
    session_id = client.sessions.create(title="My agent")
"""

from .client import PromptAsyncResult, SyncTandemClient, TandemClient
from .stream import is_run_terminal_event
from .types import (
    AgentTeamApprovalsResponse,
    AgentTeamInstance,
    AgentTeamInstancesResponse,
    AgentTeamMissionsResponse,
    AgentTeamSpawnApproval,
    AgentTeamSpawnResponse,
    AgentTeamTemplate,
    AgentTeamTemplatesResponse,
    AgentTeamTemplateWriteResponse,
    AutomationWebhookDataClass,
    AutomationWebhookDeleteResponse,
    AutomationWebhookDelivery,
    AutomationWebhookDeliveryCounts,
    AutomationWebhookDeliveryListResponse,
    AutomationWebhookDeliveryResponse,
    AutomationWebhookDeliveryStatus,
    AutomationWebhookProviderMetadata,
    AutomationWebhookProviderPolling,
    AutomationWebhookProviderVerification,
    AutomationWebhookRiskTier,
    AutomationWebhookSecretStatus,
    AutomationWebhookTrigger,
    AutomationWebhookTriggerCreateInput,
    AutomationWebhookTriggerListResponse,
    AutomationWebhookTriggerResponse,
    AutomationWebhookTriggerSecretResponse,
    AutomationWebhookTriggerUpdateInput,
    ArtifactRecord,
    AutomationV2ListResponse,
    AutomationV2FlowNode,
    AutomationV2Record,
    AutomationV2RunListResponse,
    AutomationV2RunRecord,
    AutomationWaitCorrelationBinding,
    AutomationWaitSpec,
    AutomationWaitTimeoutPolicy,
    AutomationWaitValueBinding,
    BrowserBinaryStatus,
    BrowserBlockingIssue,
    BrowserInstallResponse,
    BrowserSmokeTestResponse,
    BrowserStatusResponse,
    IncidentMonitorApprovalPolicy,
    IncidentMonitorAuthorityInventory,
    IncidentMonitorAuthorityInventoryResponse,
    IncidentMonitorAssessmentProbeCounts,
    IncidentMonitorAssessmentProbeResult,
    IncidentMonitorAssessmentProbeRunResponse,
    IncidentMonitorAssessmentReportResponse,
    IncidentMonitorDeploymentCardsResponse,
    IncidentMonitorConfigResponse,
    IncidentMonitorConfigRow,
    IncidentMonitorDestinationConfig,
    IncidentMonitorDestinationKind,
    IncidentMonitorDestinationReadiness,
    IncidentMonitorDraftListResponse,
    IncidentMonitorDraftRecord,
    IncidentMonitorIncidentListResponse,
    IncidentMonitorIncidentRecord,
    IncidentMonitorPostListResponse,
    IncidentMonitorPostRecord,
    IncidentMonitorPostureChecksResponse,
    IncidentMonitorPostureCounts,
    IncidentMonitorPostureFinding,
    IncidentMonitorPostureRuleState,
    IncidentMonitorRouteConfig,
    IncidentMonitorRoutePreviewMatch,
    IncidentMonitorRoutePreviewResponse,
    IncidentMonitorSafetyDefaults,
    IncidentMonitorStatusResponse,
    IncidentMonitorStatusRow,
    CoderArtifactRecord,
    CoderArtifactsResponse,
    CoderGithubRef,
    CoderGithubProjectBinding,
    CoderGithubProjectInboxItem,
    CoderGithubProjectInboxResponse,
    CoderGithubProjectIntakeResponse,
    CoderGithubProjectRef,
    CoderGithubProjectStatusMapping,
    CoderGithubProjectStatusOption,
    CoderMemoryCandidateRecord,
    CoderMemoryCandidatesResponse,
    CoderMemoryHitRecord,
    CoderMemoryHitsResponse,
    CoderProjectBindingGetResponse,
    CoderProjectBindingPutResponse,
    CoderProjectBindingRecord,
    CoderRepoBinding,
    CoderRunGetResponse,
    CoderRunRecord,
    CoderRunsListResponse,
    ChannelConfigEntry,
    ChannelStatusEntry,
    ChannelToolPreferences,
    ChannelVerifyResponse,
    ChannelsConfigResponse,
    ChannelsStatusResponse,
    DefinitionCreateResponse,
    DefinitionListResponse,
    EngineEvent,
    EngineMessage,
    ErrorCode,
    ErrorEnvelope,
    MemoryAuditEntry,
    MemoryAuditResponse,
    MemoryItem,
    MemoryListResponse,
    MemoryPromoteResponse,
    MemoryPutResponse,
    MemorySearchResponse,
    MemorySearchResult,
    MessagePart,
    MissionCreateResponse,
    MissionEventResponse,
    MissionGetResponse,
    MissionListResponse,
    MissionRecord,
    GoalRunLink,
    GoalLimitAction,
    GoalPolicy,
    LongRunningGoal,
    LongRunningGoalArtifact,
    LongRunningGoalArtifactsResponse,
    LongRunningGoalBudgetsResponse,
    LongRunningGoalBudgetUsage,
    LongRunningGoalCancelInput,
    LongRunningGoalCancelResponse,
    LongRunningGoalCompletionInput,
    LongRunningGoalCompletionResponse,
    LongRunningGoalControlResponse,
    LongRunningGoalEventsResponse,
    LongRunningGoalGraphResponse,
    LongRunningGoalHandoffDecisionInput,
    LongRunningGoalHandoffDecisionResponse,
    LongRunningGoalHandoffEmitInput,
    LongRunningGoalHandoffTransitionResponse,
    LongRunningGoalHandoffsResponse,
    LongRunningGoalListResponse,
    LongRunningGoalResponse,
    LongRunningGoalRun,
    LongRunningGoalRunsResponse,
    LongRunningGoalStartInput,
    LongRunningGoalStartResponse,
    StatefulGoalAutomationRunRecord,
    StatefulGoalAutomationRunStatus,
    LongRunningGoalStatus,
    LongRunningGoalWaitResolveInput,
    LongRunningGoalWaitResponse,
    LongRunningGoalWaitsResponse,
    OrchestrationArtifactContract,
    OrchestrationArtifactRef,
    OrchestrationAggregateResponse,
    OrchestrationCanvasPosition,
    OrchestrationEdgeSpec,
    OrchestrationGoalLimitAction,
    OrchestrationGoalPolicy,
    OrchestrationDraftInput,
    OrchestrationListResponse,
    OrchestrationNodeSpec,
    OrchestrationPublishResponse,
    OrchestrationRefreshReferencesResponse,
    OrchestrationStaleReferencesResponse,
    OrchestrationSpec,
    OrchestrationStatus,
    OrchestrationSummary,
    OrchestrationTenantContext,
    OrchestrationTenantSource,
    OrchestrationTerminalNodeSpec,
    OrchestrationTerminalOutcome,
    OrchestrationTransitionApprovalPolicy,
    OrchestrationTransitionPreviewInput,
    OrchestrationTransitionPreviewResponse,
    OrchestrationValidationIssue,
    OrchestrationValidationReport,
    OrchestrationValidationResponse,
    OrchestrationVersionResponse,
    OrchestrationVersionSummary,
    OrchestrationVersionsResponse,
    OrchestrationWaitNodeSpec,
    OrchestrationWorkflowNodeSpec,
    OrchestrationWorkflowReference,
    StatefulGoalCancelOutcome,
    StatefulGoalEventRecord,
    StatefulGoalCursorEvent,
    StatefulGoalWaitRecord,
    StatefulRuntimeScope,
    StatefulWaitKind,
    StatefulWaitStatus,
    StatefulWaitTimeoutPolicy,
    WorkflowHandoff,
    WorkflowHandoffStatus,
    PermissionRequestRecord,
    PermissionRule,
    PermissionSnapshotResponse,
    ProviderCatalog,
    ProviderConfigEntry,
    ProviderEntry,
    ProviderModelEntry,
    ProvidersConfigResponse,
    PromptFilePartInput,
    PromptPartInput,
    PromptTextPartInput,
    QuestionRecord,
    QuestionsListResponse,
    ResourceListResponse,
    ResourceRecord,
    ResourceWriteResponse,
    RoutineHistoryEntry,
    RoutineHistoryResponse,
    RoutineRecord,
    RunArtifactsResponse,
    RunNowResponse,
    RunRecord,
    SessionDiff,
    SessionListResponse,
    SessionRecord,
    SessionRunState,
    SessionRunStateResponse,
    SessionTodo,
    SkillImportResponse,
    SkillRecord,
    SkillsListResponse,
    SkillTemplate,
    SkillTemplatesResponse,
    SystemHealth,
    ToolExecuteResult,
    ToolSchema,
    WorkflowHookListResponse,
    WorkflowHookRecord,
    WorkflowListResponse,
    WorkflowPlan,
    WorkflowPlanApplyResponse,
    WorkflowPlanChatMessage,
    WorkflowPlanChatResponse,
    WorkflowPlanConversation,
    WorkflowPlanGetResponse,
    WorkflowPlanInputRef,
    WorkflowPlanImportPreviewResponse,
    WorkflowPlanImportResponse,
    WorkflowPlanOutputContract,
    WorkflowPlanPackBuilderExportRequest,
    WorkflowPlanPackBuilderExportResult,
    WorkflowPlanPreviewResponse,
    WorkflowPlanStep,
    WorkflowRecord,
    WorkflowRunListResponse,
    WorkflowRunRecord,
)

__all__ = [
    # Clients
    "TandemClient",
    "SyncTandemClient",
    "PromptAsyncResult",
    "PromptPartInput",
    "PromptTextPartInput",
    "PromptFilePartInput",
    "ErrorCode",
    "ErrorEnvelope",
    # Health
    "SystemHealth",
    "BrowserBlockingIssue",
    "BrowserBinaryStatus",
    "BrowserStatusResponse",
    "BrowserInstallResponse",
    "BrowserSmokeTestResponse",
    "AutomationV2FlowNode",
    "AutomationWaitCorrelationBinding",
    "AutomationWaitSpec",
    "AutomationWaitTimeoutPolicy",
    "AutomationWaitValueBinding",
    # Orchestrations
    "OrchestrationArtifactContract",
    "OrchestrationCanvasPosition",
    "OrchestrationEdgeSpec",
    "OrchestrationGoalLimitAction",
    "OrchestrationGoalPolicy",
    "OrchestrationAggregateResponse",
    "OrchestrationDraftInput",
    "OrchestrationListResponse",
    "OrchestrationNodeSpec",
    "OrchestrationPublishResponse",
    "OrchestrationRefreshReferencesResponse",
    "OrchestrationStaleReferencesResponse",
    "OrchestrationSpec",
    "OrchestrationStatus",
    "OrchestrationSummary",
    "OrchestrationTenantContext",
    "OrchestrationTenantSource",
    "OrchestrationTerminalNodeSpec",
    "OrchestrationTerminalOutcome",
    "OrchestrationTransitionApprovalPolicy",
    "OrchestrationTransitionPreviewInput",
    "OrchestrationTransitionPreviewResponse",
    "OrchestrationValidationIssue",
    "OrchestrationValidationReport",
    "OrchestrationValidationResponse",
    "OrchestrationVersionResponse",
    "OrchestrationVersionSummary",
    "OrchestrationVersionsResponse",
    "OrchestrationWaitNodeSpec",
    "OrchestrationWorkflowNodeSpec",
    "OrchestrationWorkflowReference",
    # Stateful Runtime Goals
    "GoalLimitAction",
    "GoalPolicy",
    "GoalRunLink",
    "LongRunningGoal",
    "LongRunningGoalArtifact",
    "LongRunningGoalArtifactsResponse",
    "LongRunningGoalBudgetsResponse",
    "LongRunningGoalBudgetUsage",
    "LongRunningGoalCancelInput",
    "LongRunningGoalCancelResponse",
    "LongRunningGoalCompletionInput",
    "LongRunningGoalCompletionResponse",
    "LongRunningGoalControlResponse",
    "LongRunningGoalEventsResponse",
    "LongRunningGoalGraphResponse",
    "LongRunningGoalHandoffDecisionInput",
    "LongRunningGoalHandoffDecisionResponse",
    "LongRunningGoalHandoffEmitInput",
    "LongRunningGoalHandoffTransitionResponse",
    "LongRunningGoalHandoffsResponse",
    "LongRunningGoalResponse",
    "LongRunningGoalRun",
    "LongRunningGoalRunsResponse",
    "LongRunningGoalStartInput",
    "LongRunningGoalStartResponse",
    "StatefulGoalAutomationRunRecord",
    "StatefulGoalAutomationRunStatus",
    "LongRunningGoalStatus",
    "LongRunningGoalWaitResolveInput",
    "LongRunningGoalWaitResponse",
    "LongRunningGoalWaitsResponse",
    "OrchestrationArtifactRef",
    "StatefulGoalCancelOutcome",
    "StatefulGoalEventRecord",
    "StatefulGoalCursorEvent",
    "StatefulGoalWaitRecord",
    "StatefulRuntimeScope",
    "StatefulWaitKind",
    "StatefulWaitStatus",
    "StatefulWaitTimeoutPolicy",
    "WorkflowHandoff",
    "WorkflowHandoffStatus",
    # Sessions
    "SessionRecord",
    "SessionListResponse",
    "SessionRunState",
    "SessionRunStateResponse",
    "SessionDiff",
    "SessionTodo",
    # Messages
    "EngineMessage",
    "MessagePart",
    # Permissions
    "PermissionRule",
    "PermissionRequestRecord",
    "PermissionSnapshotResponse",
    # Questions
    "QuestionRecord",
    "QuestionsListResponse",
    # Providers
    "ProviderEntry",
    "ProviderModelEntry",
    "ProviderCatalog",
    "ProviderConfigEntry",
    "ProvidersConfigResponse",
    # Channels
    "ChannelConfigEntry",
    "ChannelStatusEntry",
    "ChannelToolPreferences",
    "ChannelVerifyResponse",
    "ChannelsConfigResponse",
    "ChannelsStatusResponse",
    # Memory
    "MemoryItem",
    "MemoryPutResponse",
    "MemorySearchResult",
    "MemorySearchResponse",
    "MemoryListResponse",
    "MemoryPromoteResponse",
    "MemoryAuditEntry",
    "MemoryAuditResponse",
    # Skills
    "SkillRecord",
    "SkillsListResponse",
    "SkillImportResponse",
    "SkillTemplate",
    "SkillTemplatesResponse",
    # Resources
    "ResourceRecord",
    "ResourceListResponse",
    "ResourceWriteResponse",
    # Workflows
    "WorkflowRecord",
    "WorkflowListResponse",
    "WorkflowRunRecord",
    "WorkflowRunListResponse",
    "WorkflowHookRecord",
    "WorkflowHookListResponse",
    # Workflow Plans
    "WorkflowPlanInputRef",
    "WorkflowPlanOutputContract",
    "WorkflowPlanStep",
    "WorkflowPlan",
    "WorkflowPlanPackBuilderExportRequest",
    "WorkflowPlanPackBuilderExportResult",
    "WorkflowPlanChatMessage",
    "WorkflowPlanConversation",
    "WorkflowPlanPreviewResponse",
    "WorkflowPlanApplyResponse",
    "WorkflowPlanChatResponse",
    "WorkflowPlanGetResponse",
    "WorkflowPlanImportPreviewResponse",
    "WorkflowPlanImportResponse",
    # Incident Monitor
    "IncidentMonitorDestinationKind",
    "IncidentMonitorApprovalPolicy",
    "IncidentMonitorDestinationConfig",
    "IncidentMonitorRouteConfig",
    "IncidentMonitorSafetyDefaults",
    "IncidentMonitorDestinationReadiness",
    "IncidentMonitorRoutePreviewMatch",
    "IncidentMonitorRoutePreviewResponse",
    "IncidentMonitorConfigRow",
    "IncidentMonitorConfigResponse",
    "IncidentMonitorStatusRow",
    "IncidentMonitorStatusResponse",
    "IncidentMonitorAuthorityInventory",
    "IncidentMonitorAuthorityInventoryResponse",
    "IncidentMonitorAssessmentProbeCounts",
    "IncidentMonitorAssessmentProbeResult",
    "IncidentMonitorAssessmentProbeRunResponse",
    "IncidentMonitorAssessmentReportResponse",
    "IncidentMonitorDeploymentCardsResponse",
    "IncidentMonitorPostureRuleState",
    "IncidentMonitorPostureCounts",
    "IncidentMonitorPostureFinding",
    "IncidentMonitorPostureChecksResponse",
    "IncidentMonitorIncidentRecord",
    "IncidentMonitorIncidentListResponse",
    "IncidentMonitorDraftRecord",
    "IncidentMonitorDraftListResponse",
    "IncidentMonitorPostRecord",
    "IncidentMonitorPostListResponse",
    # Routines & Automations
    "RoutineRecord",
    "DefinitionListResponse",
    "DefinitionCreateResponse",
    "RunNowResponse",
    "RunRecord",
    "RunArtifactsResponse",
    "RoutineHistoryEntry",
    "RoutineHistoryResponse",
    # Agent Teams
    "AgentTeamTemplate",
    "AgentTeamTemplatesResponse",
    "AgentTeamTemplateWriteResponse",
    "AgentTeamInstance",
    "AgentTeamInstancesResponse",
    "AgentTeamMissionsResponse",
    "AgentTeamSpawnApproval",
    "AgentTeamApprovalsResponse",
    "AgentTeamSpawnResponse",
    # Automations V2
    "AutomationV2Record",
    "AutomationV2ListResponse",
    "AutomationV2RunRecord",
    "AutomationV2RunListResponse",
    "AutomationWebhookDataClass",
    "AutomationWebhookRiskTier",
    "AutomationWebhookDeliveryStatus",
    "AutomationWebhookSecretStatus",
    "AutomationWebhookDeliveryCounts",
    "AutomationWebhookProviderVerification",
    "AutomationWebhookProviderPolling",
    "AutomationWebhookProviderMetadata",
    "AutomationWebhookTrigger",
    "AutomationWebhookDelivery",
    "AutomationWebhookTriggerCreateInput",
    "AutomationWebhookTriggerUpdateInput",
    "AutomationWebhookTriggerListResponse",
    "AutomationWebhookTriggerResponse",
    "AutomationWebhookTriggerSecretResponse",
    "AutomationWebhookDeleteResponse",
    "AutomationWebhookDeliveryListResponse",
    "AutomationWebhookDeliveryResponse",
    # Coder
    "CoderRepoBinding",
    "CoderGithubRef",
    "CoderGithubProjectStatusOption",
    "CoderGithubProjectStatusMapping",
    "CoderGithubProjectBinding",
    "CoderGithubProjectRef",
    "CoderProjectBindingRecord",
    "CoderProjectBindingGetResponse",
    "CoderProjectBindingPutResponse",
    "CoderGithubProjectInboxItem",
    "CoderGithubProjectInboxResponse",
    "CoderGithubProjectIntakeResponse",
    "CoderRunRecord",
    "CoderRunsListResponse",
    "CoderRunGetResponse",
    "CoderArtifactRecord",
    "CoderArtifactsResponse",
    "CoderMemoryHitRecord",
    "CoderMemoryHitsResponse",
    "CoderMemoryCandidateRecord",
    "CoderMemoryCandidatesResponse",
    # Missions
    "MissionRecord",
    "MissionCreateResponse",
    "MissionListResponse",
    "MissionGetResponse",
    "MissionEventResponse",
    # Tools
    "ToolSchema",
    "ToolExecuteResult",
    # Events
    "EngineEvent",
    "is_run_terminal_event",
    "ArtifactRecord",
]
