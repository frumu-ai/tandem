...
- `GOV-B6` Governance recheck on launch + stale auto-resume
  - Status: `research` (route decided — ready to implement as B6a)
  - Priority: P1
  ...
  - Files (for B6a): `crates/tandem-server/src/app/state/app_state_impl_parts/part05.rs` (`claim_specific_automation_v2_run`, `auto_resume_stale_reaped_runs`).
  - Verification (planned): premium — spend-pause an agent → claim holds as `Paused + GuardrailStopped`, override approval resumes; OSS — queued run claims to `Running` (no-op).
