...
- `GOV-B5` Channel Approve-by-default fallback + button step-up + tenant binding
  ...
  - Progress (B5b): replaced the global env PIN (slash-only) with a **per-identity, expiring step-up grant** checkable on the button/interaction path. New ephemeral `channel_step_up_grants` store on `AppState` + `grant_channel_step_up`/`channel_step_up_active`; control-panel issuance endpoint `POST /channels/step-up` (`{channel,user_id,ttl_seconds?}`, default 5-min TTL); per-channel opt-in `require_approval_step_up` config (`channel_requires_approval_step_up`, default **off** so existing flows are unchanged). All three interaction approve handlers (slack/telegram/discord) now require an active per-identity step-up when the channel opts in. The legacy slash env-PIN (`reconfigure_step_up_satisfied`, in `tandem-channels`) is left intact as-is (separate crate; documented legacy).
  - Files (B5b): `app/state/mod.rs` + `app_state_impl_parts/part01.rs` (store field); `channel_user_capabilities.rs` (`grant_channel_step_up`, `channel_step_up_active`, `channel_requires_approval_step_up`); `channel_enrollment.rs` (`channel_step_up` endpoint); `router.rs` (route); `slack/telegram/discord_interactions.rs` (enforcement).
  - Verification (B5b): `step_up_grant_is_active_until_expiry`, `require_approval_step_up_config_defaults_off`; 20 channel-cap + interaction-handler tests green (no regression). Decision D-B5.2 resolved as opt-in (default off) to avoid imposing approval friction by default; an operator raises the bar per channel.
  - Progress (B5c): the interaction handlers adopt the **run's** tenant context but never verified the channel was bound to it — a channel user could act on another tenant's run by id. Added `channel_bound_tenant` (reads `/channels/{channel}/tenant` = `{org_id, workspace_id}`); each decision-dispatch path (slack inline, telegram/discord helper `dispatch_decision`) now rejects when a bound channel targets a run in a different tenant. Decision D-B5.3 resolved minimally: binding is **opt-in config**, and an unset binding means unbound (channel adopts the run's tenant unchanged) — so single-tenant/local is unaffected and no record migration is needed (capability records are not re-keyed; the action-scope check is the security boundary).
  - Files (B5c): `app/state/principals/channel_identity.rs` (`channel_bound_tenant`); `slack/telegram/discord_interactions.rs` (binding check at dispatch).
  - Verification (B5c): `channel_bound_tenant_parses_config_and_defaults_unbound`, `channel_is_open_to_all_detects_wildcard`; 15 interaction-handler tests green.
  - Status: **B5 complete** (B5a approve-by-default, B5b per-identity step-up on buttons, B5c tenant binding).
