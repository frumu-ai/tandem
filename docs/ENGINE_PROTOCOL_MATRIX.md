...
| Plans panel -> `list_plans`                                                          | Returns plan metadata from `.tandem/plans` (plus legacy `.opencode/plans` read fallback) | implemented | Command registered in `apps/tandem-desktop/src-tauri/src/lib.rs` invoke handler.                               |
