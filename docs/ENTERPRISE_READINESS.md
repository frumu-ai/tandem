...
- **Required dispatch receipts and durable protected audit:** The central dispatcher writes a correlated policy-decision receipt before returning an allow, denial, or approval-required result, then records execution-started and execution-completed/failed phases only after authority exists. Approval-required is a distinct pending receipt state and never calls the tool. Receipt failure blocks the next transition; denial or pending-receipt persistence failure remains blocked and returns an explicit error. Consequential external dispatch also reserves a durable pre-send outbox record. Protected events such as approvals, provider secret changes, MCP activity, governance events, and coder transitions use sequence-numbered, key-ID-bound HMAC-chained, fsynced JSONL audit envelopes with externally anchored heads. These operational receipts are distinct from the separate cryptographically signed approval-receipt contract, and the repository does not establish transactional atomicity between every external/business effect and its final audit append.
...
- **Parameter-aware inherited policy enforcement:** Published `EnterprisePolicyRule` records can apply bounded `permission_predicates/v1` expressions to exact tool arguments before inherited allow, deny, or approval-required resolution. The evaluator supports typed scalar, host/domain, fixed-precision amount, path, repository, existence, and array-length conditions with three-valued fail-closed behavior. Runtime decisions retain the effective policy snapshot and emit at most 32 condition rows with stable result/reason codes plus deployment-scoped HMAC expression, selector, and permitted selected-value digests. Raw selectors, operands, selected values, request arguments, paths, repository URLs, and email local parts are not copied into this evidence; low-cardinality value digests are omitted. Hosted/enterprise predicate decisions fail closed unless `TANDEM_AUDIT_HMAC_KEY`, `TANDEM_AUDIT_HMAC_KEY_FILE`, or `TANDEM_AUDIT_HMAC_KEYRING_FILE` supplies the audit authority.
- **Tenant and global no-code policy authoring:** Enterprise admins can use the Control Panel Policy Studio and enterprise APIs to create and edit drafts, validate field-level errors, preview the effective inherited winner, publish, disable, and supersede tenant-scoped policies. Enterprise/global admins can explicitly create tenantless enterprise-scope rules; tenant/workspace admins cannot create or mutate them. Supersession uses the same canonical tenant-ID matching and deployment-presence semantics as runtime enforcement. Mutation persistence and protected audit writes are required together, with rollback on audit failure.
- **First-class approval-required tool dispatch:** `ToolDispatchPolicyOutcome` distinguishes `approval_required` from terminal denial and carries the policy decision, winning policy/rule version, approval class, optional approval-request handle, and deployment-HMAC exact-action binding. Engine/native, batch subcall, global HTTP, server-backed CLI, and bridged MCP paths preserve the central dispatcher result. The existing action-gate store deduplicates exact scope/action/policy requests and atomically consumes one matching, unexpired receipt; argument, tenant, policy-version, connector-generation, expiry, or replay drift fails closed.
- **Versioned starter policies:** The enterprise contract, APIs, and Policy Studio ship CRM, finance, and coding starter templates. Deployments can instantiate a version, preview the effective result, apply only declared override fields, upgrade, and roll back while non-overridable denies remain authoritative.
...
> The reviewed runtime carries substantial enterprise authority primitives: durable stateful runs, tenant-aware records, centrally dispatched and receipted server tools, scoped and principal-bound MCP connections, governed approval gates, tenant and global parameter-aware policy authoring, versioned starter policies, first-class pending-approval dispatch, privacy-preserving predicate decision evidence, intra-tenant authority modeling, explicit cross-tenant grant contracts, artifact validation, retrieval/egress controls, HMAC-chained protected audit records with externally anchored heads, verifiable generic evidence exports, PostgreSQL storage options, and a reference enterprise ingestion path. These are repository capabilities, not proof of hosted deployment, non-bypassability, external control acceptance, or operational maturity. Full enterprise identity, hosted RBAC administration, OIDC, SCIM, turnkey SIEM integrations, private-sidecar enforcement, managed KMS custody across every signing/connector lane, and SOC2 remain roadmap items or unverified operational claims.

Approval gates are runtime control points, not a complete enterprise identity boundary by themselves. For regulated or customer-impacting actions, Tandem should fail closed unless the runtime can verify tenant, policy, approval, proposed-action identity, and capability evidence at the protected tool call. Every reviewed production server dispatch now requires correlated durable policy and execution receipts; authored predicate approvals and the egress-DLP path atomically consume an exact-action approval once. Tandem also has a cryptographic approval-receipt verifier contract and a fintech path that matches approved gate history to tenant, tool, exact action hash, and expiry. These are different receipt layers: the repository does not establish that every protected tool consumes a cryptographically signed approval receipt. Universal signed-receipt consumption and enterprise required-mode private-sidecar enforcement remain open.
