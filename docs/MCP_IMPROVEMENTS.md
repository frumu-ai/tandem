...
- GitHub Projects discovery, inbox listing, and item status sync should flow through the connected MCP server.
- The local `git` tool remains responsible for clone, checkout, worktrees, commits, and diffs.

Operator note:

- Manual `mcp add` should not be required for the GitHub Projects workflow.
- Secret material should stay out of plain config files and logs.
