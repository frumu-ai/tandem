...
| Control plane to runtime | Metadata-bearing Ed25519 public keyring plus a shared durable replay ledger. | Assertion private keys, atomic keyring publication, replay-store availability, issuer/audience/lifetime policy, and clock sync. |
...
| Replayed context assertion           | Hosted replicas coordinate `bound` or `one_shot` decisions through a required transactional durable ledger, retain records through expiry plus grace, and fail closed on corruption, outage, or capacity exhaustion. | Operators must place every replica on the same locking-capable shared path and monitor backend health.               |
...
| Misconfigured assertion key rotation | Hosted mode rejects legacy keys; startup and explicit admin reload validate secure files and atomically publish complete immutable generations while retaining last-known-good state on failure. | Operators must keep signer cutover and verifier overlap aligned and use the audited explicit reload procedure.        |
