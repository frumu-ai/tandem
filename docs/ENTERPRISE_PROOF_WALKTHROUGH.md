...
- **Protected audit and evidence export:** `crates/tandem-server/src/audit.rs`, `http/audit_stream.rs`, and `http/context_run_ledger.rs` implement HMAC-chained durable records with externally anchored heads, a ledger manifest, deterministic NDJSON export, and structured run-governance evidence packages.
