...
- **Workflow learning API, state, runtime, candidate-generation, planner-revision, distillation, and evidence coverage**: Added focused HTTP, state, runtime, and memory-layer coverage for workflow learning candidate listing, review, promotion, revision-spawn guardrails, candidate dedupe, status transitions, approved-learning prompt injection, run-summary persistence, terminal-run candidate generation thresholds, planner revision session creation, distillation writer accounting, and richer candidate evidence payloads so the new surfaces keep deterministic behavior as the feature hardens.
