...
- **Workflow learning API, state, runtime, and candidate-generation coverage**: Added focused HTTP, state, and runtime coverage for workflow learning candidate listing, review, promotion, revision-spawn guardrails, candidate dedupe, status transitions, approved-learning prompt injection, run-summary persistence, and terminal-run candidate generation thresholds so the new surfaces keep deterministic behavior as the feature hardens.
