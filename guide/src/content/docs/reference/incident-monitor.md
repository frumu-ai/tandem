...
Use `GET /incident-monitor/security/scenario-packs` to describe the built-in pack and `POST /incident-monitor/security/scenario-packs` to run it (or a supplied custom pack) in dry-run. Scenario packs are versioned, data-driven descriptions of production-mirroring governance edge cases — regulatory escalation, prompt injection, excessive agency, cross-tenant forgery, unsafe/unready destinations, recurring denied-action escalation — that exercise the destination router's route preview, approval gate, and readiness gates. Running a pack never mutates external systems (`mutates_external_systems` is always `false`), is admin-only, and rejects scoped intake keys. A failing scenario surfaces a governance gap and carries a `finding_id`. Results also fold into the assessment report under `sections.adversarial_scenario_packs`.

## Governance Maturity Metrics

Use `POST /incident-monitor/security/governance-metrics` to compute redacted governance-health metrics plus behavioral drift in dry-run mode. Metrics include governance confidence (high-risk incidents with a complete audit trail), authority-boundary compliance, escalation-pathway utilization, route-readiness compliance, receipt completeness, and recurring-incident rate. Each metric reports numerator, denominator, rate, threshold, and whether it breached; metrics with no data are `evaluable: false` rather than a breach. Behavioral drift compares the current window against the prior baseline window and flags signals (publish-failure, denied-action, escalation, per-category incident share) that move by more than `drift_rate_delta_max`. Thresholds ship with production-safe defaults and can be overridden per request. Output is redacted (counts, rates, ids, hashes only), admin-only, and folds into the assessment report under `sections.governance_maturity_metrics`.

## Continuous Reassessment

Use `POST /incident-monitor/security/reassessments` to run a reassessment now and `GET /incident-monitor/security/reassessments` to list recent records plus per-scope schedule status. The Incident Monitor re-runs its posture, governance-metric, and drift surfaces on a cadence and on selected configuration/runtime change events, producing a versioned `ReassessmentRecord` that compares current findings against the previous run (`new` / `recurring` / `resolved` fingerprints, with recurring findings keeping `first_seen_at_ms` and an `occurrence_count`). Change events are detected by diffing the governance-relevant sections of the Incident Monitor config on save (destination/route, monitored source, tenant boundary, model policy, MCP inventory, approval policy). Everything runs read-only (`mutates_external_systems: false`); escalation still flows through the normal draft/approval/routing path. Cadence, overdue grace, and change triggers are tunable under `config.incident_monitor.reassessment`. Schedule status (`last_completed_at_ms`, `next_due_at_ms`, `overdue`) also folds into the deployment-cards payload under `reassessment.schedule`. Admin-only; a background scheduler runs due and change-triggered scopes automatically.
...
- Destination readiness is enforced fail-closed by default. `block_unready_destinations` defaults to `true`, and readiness is re-checked at delivery time after approval clears; `Auto` and `ManualPublish` publishes always block an unready destination, while setting the flag to `false` only frees `Recovery`-mode publishes. A blocked delivery emits a `destination_not_ready` audit event and produces no receipt.
