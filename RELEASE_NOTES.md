...
- **Workflow learning API, state, and runtime coverage**: Added focused HTTP, state, and runtime coverage for workflow learning candidate listing, review, promotion, revision-spawn guardrails, candidate dedupe, status transitions, approved-learning prompt injection, and run-summary persistence so the new surfaces keep deterministic behavior as the feature hardens.
